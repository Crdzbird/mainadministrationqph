...
namespace QPH_MAIN.Core.Services
{
    public class UserService : IUserService
    {
        ...
        public Task<bool> ActivateUserAccount(string code)
        {
        ...
        {
            string EmailOriginal = "desarrolloqph1@qph.com.ec";
            string password = "Qph2020.";
            // string url = urlDomain+ "/AccountRecovery/Recovery/?token="+token;
            MailMessage mailMessage = new MailMessage(EmailOriginal, destination, "Account Activation",
                "<p>Account Acctivation: </p> " + message);
            mailMessage.IsBodyHtml = true;
            SmtpClient smtpClient = new SmtpClient("smtp.office365.com");
            smtpClient.EnableSsl = true;
            smtpClient.UseDefaultCredentials = true;
            smtpClient.Port = 587;
            smtpClient.Credentials = new NetworkCredential(EmailOriginal, password);
            smtpClient.Send(mailMessage);
            smtpClient.Dispose();
        }

        public string GenerateActivationCode(int length = 12, string allowedChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789~[!]+-")
        {
